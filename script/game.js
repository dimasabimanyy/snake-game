...
import { checkSelfCollision, spawnFood, drawText, drawSnake } from "./helper.js";
...
let food = spawnFood(CANVAS_SIZE, BOX_SIZE); // Makanan biasa
...
function handleBigFood() {
  if (!bigFood && Math.random() < BIG_FOOD_SPAWN_CHANCE) {
    bigFood = spawnFood(CANVAS_SIZE, BOX_SIZE);
...
function drawGame() {
  ...
  if (isGameOver) {
    ...
    drawText(ctx, "Tekan Enter untuk Restart", CANVAS_SIZE / 5, CANVAS_SIZE / 1.9);
    return;
  }

  drawFood(food); // Gambar makanan biasa
  if (bigFood) drawFood(bigFood, BOX_SIZE / 1.5, "gold", true); // Gambar big food
  drawSnake(snake, ctx, BOX_SIZE); // Gambar ular
  drawText(ctx, `Score: ${score}`, 10, 25); // Gambar skor
}
...
function moveSnake() {
  ...
  if (head.x === food.x && head.y === food.y) {
    score++;
    sound.play();
    food = spawnFood(CANVAS_SIZE, BOX_SIZE);
